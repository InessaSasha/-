...
# Значения строк будут перебираться из объединённых вместе списков first_strings
# и second_strings. Условие записи пары в словарь - чётная длина строки.

new_list = [item for sublist in zip(first_strings, second_strings) for item in sublist]

third_result = {x:len(x) for x in new_list if len(x) // 2}
print(third_result)
